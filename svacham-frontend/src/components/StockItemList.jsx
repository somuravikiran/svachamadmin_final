...
function StockItemList(){
  ...
  const handleView=async(r)=>{ setLoading(true); try{ const fresh = await stockItemAPI.getStockItemBySeq(r.seq); setSelected(fresh || r);}catch(err){ console.error(err); setSelected(r);}finally{ setLoading(false); setShowView(true);} };
  const handleDelete=async(seq)=>{ if(!window.confirm('Delete this record?')) return; try{ await stockItemAPI.deleteStockItemBySeq(seq); setSuccess('Deleted'); fetchItems(); fetchSummary(); setTimeout(()=>setSuccess(''),3000);}catch(err){ console.error(err); setError('Delete failed'); }};
  const handleSubmit=async(form,seq)=>{ try{ if(seq) await stockItemAPI.updateStockItemBySeq(seq,form); else await stockItemAPI.createStockItem(form); fetchItems(); fetchSummary(); setTimeout(()=>setSuccess('Saved'),2000);}catch(err){ console.error(err); setError('Save failed'); }};
    ...
    <div className="stockitem-container">
      ...
      <div className="table-wrap">{loading? <div className="loading">Loading...</div> : (
        ...
        <tbody>{filtered.map(r=> (
          <tr key={r.seq || r.id}><td>{r.seq || r.id}</td><td>{r.itemName}</td><td>{r.itemCategory}</td><td>{r.quantity}</td><td>{r.totalStock}</td><td>{r.usedStock}</td><td>{r.remainingStock}</td><td><span className={`status ${r.status}`}>{r.status}</span></td><td className="actions"><button className="act view" onClick={()=>handleView(r)}>👁️ View</button><button className="act edit" onClick={()=>handleEdit(r)}>✏️ Edit</button><button className="act del" onClick={()=>handleDelete(r.seq || r.id)}>🗑️ Delete</button></td></tr>
        ))}</tbody></table>)}</div>
